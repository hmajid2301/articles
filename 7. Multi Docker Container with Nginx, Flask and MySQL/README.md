...
tags: ['docker', 'python', 'docker-compose', 'flask', 'nginx']
...
Another big advantage of Docker is that I don't need to download all the dependencies on my own machine directly. To build and run an entire application all you need is a dockerfile. I can easily set up my own development environment simply by using Docker. All Docker does is it executes a set of instructions so since the instructions are the same, the environment Docker will create will also be the same.
...
that has Nginx, Flask and Postgres but I prefer to split the application up, into its core components the web server (Nginx), the application (Flask) and the database (Postgres). One advantage being it is easier replace components of the application if required. Another advantage being it easier to isolate problems and bugs with the application.
...
In theory, you could simply copy and install the requirements.txt and copy all the source code to the `/app` folder. However, I prefer to generate and install the actual Python package because I think it's cleaner and you only have to copy a single `tar` file. However, this does require you to run the command to generate the `dist` folder before you try to build the Docker image.

### uwsgi.ini

![uwsgi.ini](images/flask_uwsgi.png)

This is the configuration file used by the uWSGI server. This is where we define which port uWSGI listens for traffic on in this cases it's 8080. **Note** that since we've defined `socket` you cannot access the uWSGI server directly. You need a web server (Nginx) to forward traffic to the uWSGI server if you wanted to use just uWSGI, you would change this option to `http`. The other import option is the `module`, we point it to our installed module example then the wsgi module and app variable. Hence the `module=example.wsgi:app`. In this example the `wsgi.py` module calls `create_app()` function which creates the Flask app.
...
This service runs our Nginx server. We call the container `nginx` for obvious reasons. We pass it the location of the dockerfile to build. The build `context: .` simply means relative to this current working directory so when we `COPY` the file we will copy them using the relative path to docker-compose.yml file. The final thing we do is publish our ports so all traffic on the host machine on port 80 is mapped to the Docker container also on port 80. We could use any port on our guest machine lets say we used 8001, then we would access the web server by going to `localhost:8001`. The final part specifies this container depends on the `app` container. So the `app` container will be run before this container.

### app

This is a relatively simple service, again we point it to our flask dockerfile and set the build context to the current folder `context: .`. Then we pass some environment variables as a file, the variables are taken from this file. This is the same `database.conf` as shown above. These variables are used to allow Python to connect to the database. Finally, we expose port 8080, again this is done for documentation purposes to people who use this container know that it expects traffic on port 8080. Since we need to connect to the database when we set up our app we depend on the database container being run first.

### database

This service doesn't have its own dockerfile but instead uses the official Postgres image. It then passes some environment variables as a file, the same file that gets passed to Flask container. We don't actually need the host or port variables but it's easier to maintain a single file in this case. We then map the host port 5432 to the guest Docker container port 5432. This is the port that Postgres listens on. Like with Nginx you can set the host port to whatever you want, but make sure you change this in `database.conf` and update `POSTGRES_PORT` variable. Finally, we mount a volume so that data is persistent. Without this volume, all your data would be lost when the container was stopped/killed. By mounting `db_volume` even when you kill the container to say update the Postgres image your data will persist.
...
One important thing to note is `docker-compose` should not really be used in production for several reasons, such as downtime when updating your Docker containers. If you're deploying to only 1 host, docker-compose should be fine but in reality, most applications require high availability and zero downtime updates, in this case, you should look at using a container orchestration tools such as Kuberenetes. So an alternative approach is to built and run each Docker container yourself, the equivalent commands for this `docker-compose.yml` file would be:
